...
function generateGrid(size) {
    ...
    allCells.forEach(cell => {
        ...
        cell.parentElement.addEventListener("click", updateSolveButtonState);
    });
...
function getColorData() {
    ...
    for (let row = 0; row < gridSize; row++) {
        ...
        for (let col = 0; col < gridSize; col++) {
            ...
            const color = (cell === null || cell === void 0 ? void 0 : cell.dataset.color) || ""; // Empty string if no color
...
function buildPayload() {
    ...
    // Row sums
    for (let row = 0; row < gridSize; row++) {
        const input = document.getElementById(`rowsum-${row}`);
        rowSums.push(Number(input.value));
    }
    // Column sums
    for (let col = 0; col < gridSize; col++) {
        const input = document.getElementById(`colsum-${col}`);
        colSums.push(Number(input.value));
    }
    // Color sum inputs (from color key)
    document.querySelectorAll(".color-option").forEach(option => {
        const colorBox = option.querySelector(".color-box");
...
function renderSolutionGrid(solution) {
